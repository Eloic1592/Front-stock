...
export const useEmplacementfunctions = (data) => {
  ...
  const [iddepot, setIddepot] = useState('1');
  // Pagination
  // Pagination
  ...
  const filtredata = filtreemplacement(data.listeemplacements, codeemp, iddepot);
...
//  Filtre de recherche
export function filtreemplacement(listeemplacements, codeemp, iddepot) {
  return listeemplacements.filter((Item) => {
    const typematch = !codeemp || Item.codeemp.toLowerCase().includes(codeemp.toLowerCase());
    let depotmatch = true;
    if (iddepot !== '1') {
      depotmatch = Item.iddepot === iddepot;
    }
    return typematch && depotmatch;
  });
}

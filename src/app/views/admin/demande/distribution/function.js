...
export const useListedistributionFunctions = (data) => {
  ...
  const handleSelectAll = (event) => {
    if (event.target.checked) {
      setSelectedIds(data.vuedistributions.map((row) => row.iddistribution));
    } else {
      setSelectedIds([]);
    }
  };
...
export function filtredistributions(vuedistributions, codearticle, datedistribution, codedepot) {
  return vuedistributions.filter((distribution) => {
